...
jq -er '.hooks.SessionStart[]?.hooks[]?.command == "${CLAUDE_PLUGIN_ROOT}/bin/rtailscale setup plugin-hook"' "${hooks_json}" >/dev/null
jq -er '.hooks.ConfigChange[]? | select(.matcher == "user_settings") | .hooks[]?.command == "${CLAUDE_PLUGIN_ROOT}/bin/rtailscale setup plugin-hook"' "${hooks_json}" >/dev/null
